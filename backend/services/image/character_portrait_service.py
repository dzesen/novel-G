"""Persistent, user-triggered character portrait generation."""
...
class PortraitJobProjection(BaseModel):
...
class CharacterPortraitService:
        ...
        *,
        ...
        if winner is None:
            raise PortraitJobNotFoundError(
                "Portrait job disappeared during state transition"
            )
        return winner, False
    ...
    async def _terminal_late_cas_or_winner(
        ...
        *,
        ...
        if winner is None:
            raise PortraitJobNotFoundError(
                "Portrait job disappeared during late reconciliation"
            )
        return winner, False
        ...
        *,
        ...
        try:
            ...
            if len(poll_result.artifacts) != 1:
                failure = ImageFailure(
                    code="execution_failed",
                    message="角色立绘任务没有返回恰好一张图片",
                    action="检查 workflow 产物节点，确保单张立绘只输出一张图片",
                    details={
            ...
            try:
                asset, _pending_anchor = (
                    await self._consume_successful_portrait(
                        owner_id=owner_id,
                        novel_id=novel_id,
                        card_id=card_id,
                        job=claimed,
        ...
        *,
        ...
        while True:
            ...
            if current is None:
                ...
                raise PortraitJobNotFoundError(
                    "Portrait job disappeared after provider submission"
                )
    ...
    async def _record_submit_failure(
        ...
        *,
        ...
        while True:
            ...
            if current is None:
                raise PortraitJobNotFoundError(
                    "Portrait job disappeared after submission failure"
                )
            if bool(current.get("is_terminal")):
                return _projection(current)
    ...
    async def _consume_successful_portrait(
        self,
        *,
        owner_id: str,
        novel_id: str,
        ...
        if len(consumption.assets) != 1:
            raise RuntimeError(
                "Managed portrait storage did not return one asset"
            )
    ...
    async def start(
        ...
        *,
        ...
        idempotency_material = {
            ...
            "seed": chosen_seed,
            "anchor_before": self._canonical_anchor(current_anchor),
            ...
            job = await self.jobs.create_job(
                {
                    ...
                    # BSON only supports signed int64; preserve the full
                    # uint64 generation seed as canonical decimal text.
        ...
        try:
            try:
                ...
                return await self._record_submit_failure(
                    owner_id=owner_id,
                    novel_id=novel_id,
                    card_id=card_id,
                    job_id=_job_id(job),
    ...
    async def poll(
        ...
        *,
        ...
        if job is None:
            raise PortraitJobNotFoundError("Portrait job not found")
        ...
        if not isinstance(raw_handle, dict):
            raise PortraitConfigurationError(
                "Portrait job has no persisted provider handle"
            )
        handle = ImageJobHandle.model_validate(raw_handle)
        resolved = await self.provider_resolver.resolve(
            usage="character_portrait",
